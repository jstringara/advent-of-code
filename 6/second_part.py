...
def move_guard(
    ...
    if not is_out_of_bounds(map, next_position):
        visited_points.append((next_position, f"{direction} -> {next_direction}"))
...
def is_in_loop(visited_points: list[tuple[tuple[int, int], str]]) -> bool:
    return len(visited_points) != len(set(visited_points))
...
def find_all_positions(map: list[str]):
    ...
    while not is_out_of_bounds(map, position) and not is_in_loop(visited_points):
...
def find_all_obstacles(map: list[str]):
    # find all visited points
    _, visited_points = find_all_positions(map)

    obstacles = []
    # put an obstacle in the position of the guard
    for position, _ in visited_points:
        i, j = position

        # skip the starting point
        if position == visited_points[0][0]:
            continue
        ...
        _, visited_points = find_all_positions(new_map)

        # check if there is a loop
        if is_in_loop(visited_points) and position not in obstacles:
...
obstacles = find_all_obstacles(test_map)

my_sum = len(obstacles)
...
obstacles = find_all_obstacles(my_map)
