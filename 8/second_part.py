...
def find_antinodes(loc_1, loc_2, grid):
    ...
    # add point until we get out of bounds
    antinode = loc_1
    multiplier = 1
    while is_point_valid(antinode, grid):
        antinodes = [antinode] + antinodes
        antinode = (
            loc_1[0] + multiplier * x_distance,
            loc_1[1] + multiplier * y_distance,
        )
        multiplier += 1
    ...
    while is_point_valid(antinode, grid):
        antinodes += [antinode]
        antinode = (
            loc_2[0] - multiplier * x_distance,
            loc_2[1] - multiplier * y_distance,
        )
        multiplier += 1
